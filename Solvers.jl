module Solver
export CN_matrix, CN_boundary_conditions, rk4, Pseudospectral_FFT
    ...
    function Pseudospectral_FFT(init_params)
        V = init_params.potential_field
        A = exp.(-im.*ħ*init_params.k_space.^2*init_params.dt/4m) 
        B = exp.(-im*V*ħ*init_params.dt/2m)
        
        init_params.ψ =  fft(init_params.ψ) .* A
        init_params.ψ =  ifft(init_params.ψ) .* B
        init_params.ψ =  fft(init_params.ψ) .* A
        init_params.ψ =  ifft(init_params.ψ)
    end 
